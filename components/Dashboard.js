...
const COINS = [
  ...
  { id: 'wrapped-steth',      symbol: 'wstETH', name: 'Wrapped stETH',     icon: 'Ξ'  },
  { id: 'kaspa',              symbol: 'KAS',    name: 'Kaspa',             icon: 'K'  },
  { id: 'internet-computer',  symbol: 'ICP',    name: 'Internet Computer', icon: '∞'  },
  { id: 'ethereum-classic',   symbol: 'ETC',    name: 'Ethereum Classic',  icon: 'Ξ'  },
  { id: 'monero',             symbol: 'XMR',    name: 'Monero',            icon: '🔒' },
  { id: 'aptos',              symbol: 'APT',    name: 'Aptos',             icon: 'A'  },
  { id: 'stellar',            symbol: 'XLM',    name: 'Stellar',           icon: '✦'  },
  { id: 'okb',                symbol: 'OKB',    name: 'OKB',               icon: 'O'  },
  { id: 'aave',               symbol: 'AAVE',   name: 'Aave',              icon: 'A'  },
  { id: 'crypto-com-chain',   symbol: 'CRO',    name: 'Cronos',            icon: 'C'  },
  { id: 'mantle',             symbol: 'MNT',    name: 'Mantle',            icon: 'M'  },
  { id: 'render-token',       symbol: 'RENDER', name: 'Render',            icon: 'R'  },
  { id: 'filecoin',           symbol: 'FIL',    name: 'Filecoin',          icon: 'F'  },
  { id: 'cosmos',             symbol: 'ATOM',   name: 'Cosmos',            icon: '⚛'  },
  { id: 'hedera-hashgraph',   symbol: 'HBAR',   name: 'Hedera',            icon: 'ℏ'  },
  { id: 'arbitrum',           symbol: 'ARB',    name: 'Arbitrum',          icon: 'A'  },
  { id: 'vechain',            symbol: 'VET',    name: 'VeChain',           icon: 'V'  },
  { id: 'immutable-x',        symbol: 'IMX',    name: 'Immutable',         icon: 'I'  },
  { id: 'optimism',           symbol: 'OP',     name: 'Optimism',          icon: 'O'  },
  { id: 'the-graph',          symbol: 'GRT',    name: 'The Graph',         icon: 'G'  },
  { id: 'algorand',           symbol: 'ALGO',   name: 'Algorand',          icon: 'A'  },
  { id: 'thorchain',          symbol: 'RUNE',   name: 'THORChain',         icon: '⚡' },
  { id: 'fantom',             symbol: 'FTM',    name: 'Fantom',            icon: 'F'  },
  { id: 'sei-network',        symbol: 'SEI',    name: 'Sei',               icon: 'S'  },
  { id: 'injective-protocol', symbol: 'INJ',    name: 'Injective',         icon: 'I'  },
  { id: 'sui',                symbol: 'SUI',    name: 'Sui',               icon: 'S'  },
  { id: 'pepe',               symbol: 'PEPE',   name: 'Pepe',              icon: '🐸' },
  { id: 'bonk',               symbol: 'BONK',   name: 'Bonk',              icon: '🐶' },
  { id: 'wormhole',           symbol: 'W',      name: 'Wormhole',          icon: 'W'  },
];
...
export default function Dashboard() {
  ...
  return (
    <div style={{ background: ghostMode ? '#000' : '#050a0e', minHeight: '100vh', backgroundImage: ghostMode ? 'none' : 'radial-gradient(ellipse at 15% 0%, rgba(0,212,255,0.05) 0%, transparent 50%), radial-gradient(ellipse at 85% 100%, rgba(0,255,136,0.03) 0%, transparent 50%)', transition: 'background 0.4s' }}>
      ...
      <main style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setShowScanner(p => !p)} style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '0.65rem', color: showScanner ? '#ff3a5c' : '#4a7a96', background: showScanner ? 'rgba(255,58,92,0.08)' : 'transparent', border: `1px solid ${showScanner ? 'rgba(255,58,92,0.4)' : '#1a3040'}`, padding: '6px 14px', cursor: 'pointer', letterSpacing: '0.1em', borderRadius: 2, transition: 'all 0.2s' }}>🔍 {showScanner ? 'HIDE' : 'SHOW'} LEAK SCANNER</button>
          <button onClick={() => setGhostMode(p => !p)} style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '0.65rem', color: ghostMode ? '#00ff88' : '#4a7a96', background: ghostMode ? 'rgba(0,255,136,0.06)' : 'transparent', border: `1px solid ${ghostMode ? 'rgba(0,255,136,0.3)' : '#1a3040'}`, padding: '6px 14px', cursor: 'pointer', letterSpacing: '0.1em', borderRadius: 2, transition: 'all 0.2s', boxShadow: ghostMode ? '0 0 12px rgba(0,255,136,0.15)' : 'none' }}>👻 {ghostMode ? 'EXIT' : 'GHOST MODE'}</button>
        </div>

        {showGold    && <GoldPanel goldData={goldData} xmrData={marketData['monero']} />}
        {showPrivacy && <PrivacyPanel xmrData={marketData['monero']} />}
        {showScanner && <PrivacyScanner />}
        ...
        {hasData && visibleCoins.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: ghostMode ? 'repeat(auto-fill, minmax(160px, 1fr))' : 'repeat(auto-fill, minmax(240px, 1fr))', gap: ghostMode ? 6 : 12 }}>
            {visibleCoins.map((coin, i) => (
              <CoinCard key={coin.id} coin={coin} data={marketData[coin.id]} prev={prevData[coin.id]} index={i} ghostMode={ghostMode} />
            ))}
          </div>
        )}
